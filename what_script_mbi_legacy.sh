...
declare -A table_script_relations=(
  ...
  ["mod_bam_reporting_ba_availabilities","MBI"]="${MBI_SCRIPT_PATH}/importData_legacy.pl -r --bam-only"
  ["hoststateevents","Central"]="/usr/share/centreon/cron/eventReportBuilder --config=/etc/centreon/conf.pm"
  ["hoststateevents","MBI"]="${MBI_SCRIPT_PATH}/importData_legacy.pl -r --ignore-databin"
  ["servicestateevents","Central"]="/usr/share/centreon/cron/eventReportBuilder --config=/etc/centreon/conf.pm"
  ["servicestateevents","MBI"]="${MBI_SCRIPT_PATH}/importData_legacy.pl -r --ignore-databin"
  ["mod_bi_hoststateevents","MBI"]="${MBI_SCRIPT_PATH}/eventStatisticsBuilder_legacy.pl -r --events-only"
  ["mod_bi_servicestateevents","MBI"]="${MBI_SCRIPT_PATH}/eventStatisticsBuilder_legacy.pl -r --events-only"
#  ["mod_bi_time","MBI"]="${MBI_SCRIPT_PATH}/perfdataStatisticsBuilder_legacy.pl -r --no-purge"
  ["mod_bi_hostavailability","MBI"]="${MBI_SCRIPT_PATH}/eventStatisticsBuilder_legacy.pl -r --no-purge --availability-only"
  ["mod_bi_serviceavailability","MBI"]="${MBI_SCRIPT_PATH}/eventStatisticsBuilder_legacy.pl -r --no-purge --availability-only"
  ["mod_bi_hgmonthavailability","MBI"]="${MBI_SCRIPT_PATH}/eventStatisticsBuilder_legacy.pl -r --no-purge --month-only --availability-only"
  ["mod_bi_hgservicemonthavailability","MBI"]="${MBI_SCRIPT_PATH}/eventStatisticsBuilder_legacy.pl -r --no-purge --month-only --availability-only"
  ["data_bin","MBI"]="${MBI_SCRIPT_PATH}/importData_legacy.pl -r --no-purge --databin-only"
  ["mod_bi_metricdailyvalue","MBI"]="${MBI_SCRIPT_PATH}/perfdataStatisticsBuilder_legacy.pl -r --no-purge"
  ["mod_bi_metricmonthcapacity","MBI"]="${MBI_SCRIPT_PATH}/perfdataStatisticsBuilder_legacy.pl -r --no-purge --month-only --no-centile"
  ["mod_bi_metrichourlyvalue","MBI"]="${MBI_SCRIPT_PATH}/perfdataStatisticsBuilder_legacy.pl -r --no-purge"
  ["mod_bi_metriccentiledailyvalue","MBI"]="/usr/share/centreon-bi/bin/centreonBIETL -rIC"
  ["mod_bi_metriccentiledailyvalue","MBI_2"]="${MBI_SCRIPT_PATH}/dimensionsBuilder_legacy.pl -r --no-purge --centile"
  ["mod_bi_metriccentiledailyvalue","MBI_3"]="${MBI_SCRIPT_PATH}/perfdataStatisticsBuilder_legacy.pl -r --centile-only"
  ["mod_bi_metriccentileweeklyvalue","MBI"]="/usr/share/centreon-bi/bin/centreonBIETL -rIC"
  ["mod_bi_metriccentileweeklyvalue","MBI_2"]="${MBI_SCRIPT_PATH}/dimensionsBuilder_legacy.pl -r --no-purge --centile"
  ["mod_bi_metriccentileweeklyvalue","MBI_3"]="${MBI_SCRIPT_PATH}/perfdataStatisticsBuilder_legacy.pl -r --centile-only"
  ["mod_bi_metriccentilemonthlyvalue","MBI"]="/usr/share/centreon-bi/bin/centreonBIETL -rIC"
  ["mod_bi_metriccentilemonthlyvalue","MBI_2"]="${MBI_SCRIPT_PATH}/dimensionsBuilder_legacy.pl -r --no-purge --centile"
  ["mod_bi_metriccentilemonthlyvalue","MBI_3"]="${MBI_SCRIPT_PATH}/perfdataStatisticsBuilder_legacy.pl -r --centile-only --month-only"
)
...
function display_scripts {
        ...
        do
                ...
                if [[ "$table_name_ds" =~ .*",".* ]]; then
                         ...
                         echo "\`\`\`"
                         echo "${script_ds} | tee /tmp/partial_rebuild_$today.log"
                         echo "\`\`\`"
                echo ""
                echo ""
        done
